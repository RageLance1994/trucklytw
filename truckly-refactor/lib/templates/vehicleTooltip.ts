...
<style>
  ...
  .truckly-pill.success { background: rgba(34,197,94,0.25); color:#86efac; }
  .truckly-pill.warning { background: rgba(234,179,8,0.25); color:#fde68a; }
  .truckly-grid {
    display: flex;
...
export function renderVehicleTooltip({
  ...
  const fuelLiters = Number.isFinite(summary.liters)
    ? Number(summary.liters).toFixed(1)
    : "-";
  const fuelPercent = Number.isFinite(summary.percent)
    ? `${(Number(summary.percent) * 100).toFixed(1)}%`
    : "-";
  const capacity = Number.isFinite(summary.capacity)
    ? `${Number(summary.capacity).toFixed(1)} L`
    : "";
  ...
  const actions = [
    { label: "Percorsi", icon: icons.route },
    { label: "Carburante", icon: icons.fuel },
    { label: "Autista", icon: icons.driver },
    { label: "Alert", icon: icons.alert },
  ];
