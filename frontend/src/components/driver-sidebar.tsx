...
export function DriverSidebar({ isOpen, onClose, selectedDriverImei }: DriverSidebarProps) {
  return (
    <aside
      className={`fixed top-16 bottom-0 right-0 z-40 w-[460px] max-w-md h-[calc(100%-4rem)] border-l border-white/10 bg-[#0e0f14] text-[#f8fafc] flex flex-col shadow-[0_24px_60px_rgba(0,0,0,0.45)] backdrop-blur truckly-sidebar transition-transform duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? "translate-x-0" : "hidden-right"}`}
      aria-hidden={!isOpen}
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-4 bg-[#0e0f14]">
        <Section
        ...
        <Section
          ...
          body="Nome, patente, e anagrafica verranno mostrati qui."
        />
        <Section
          title="Contatti"
          body="Email, telefono e note mostreranno qui."
        />
        <Section
          title="Stato & disponibilità"
          body="Turni, disponibilità e eventi recenti compariranno qui."
        />
      </div>
    </aside>
  );
